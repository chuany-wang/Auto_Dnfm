...
class GameAction:
    def __init__(self, ctrl: GameControl):
        ...
        self.templates = [cv.imread(f'dnfm\\img\\SZT_{status}.png', cv.IMREAD_GRAYSCALE) for status in
                          ['no', 'yes', 'ing']]
    ...
    def categorize_objects(self, result: List[Detect_Object]) -> Tuple[
        List[Detect_Object], List[Detect_Object], List[Detect_Object], List[Detect_Object], List[Detect_Object]]:
    ...
    def determine_action(self, hero: Detect_Object, monster: List[Detect_Object], item: List[Detect_Object],
                         arrow: List[Detect_Object], gate: List[Detect_Object], AGAIN: bool) -> Tuple[
        Optional[Detect_Object], str]:
    ...
    def get_user_position(self, image: np.ndarray) -> Optional[int]:
        ...
        region_ranges = [(c * cell_width, r * cell_height, (c + 1) * cell_width, (r + 1) * cell_height) for r in
                         range(num_rows) for c in range(num_cols)]
